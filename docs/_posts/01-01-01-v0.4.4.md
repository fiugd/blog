...
The main focus should be on removing operations which incur a big performance hit, like cloning all file contents versus downloading as used.
I'll also be adding support for git command options clone and config to coinside with "terminal-first" effort mentioned elsewhere.

Left to finish:
	- "root" terminal; change the way terminal determines currently loaded service
	- git config, esp at root (for intial clone and sacing user info/creds)
	- other git commands: list, open, close
<!-- more -->
...
- [ ] refactor modules into their own folders, remove "shared" folder (or drastically reduce size)
	- [ ] worker transpiling and service worker build system
